...
namespace Reductech.EDR.Connectors.Nuix.processes.meta
{
    ...
    public sealed class NuixProcessCombiner : IProcessCombiner
    {
        ...
        {
            var pair = RubyBlockConversion.TryConvert(p1)
                .BindCast<IRubyBlock, IUnitRubyBlock>()
                .Compose(()=> RubyBlockConversion.TryConvert(p2).BindCast<IRubyBlock, IUnitRubyBlock>());
            ...
            if(pair.Value.Item1 is SequenceRubyBlock srb)
                blocks.AddRange(srb.Blocks);
            else
